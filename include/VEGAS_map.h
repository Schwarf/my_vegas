...
private:
    ...
    std::vector<std::vector<double> > x_edges; // The edges in x, size = dimensions x N_EDGES;
    std::vector<std::vector<double> > dx_steps; // The step for each interval, size = dimensions x N_INTERVALS;

    std::vector<std::vector<double> > x_edges_last; // The edges in x, size = dimensions x N_EDGES;
    std::vector<std::vector<double> > dx_steps_last; // The step for each interval, size = dimensions x N_INTERVALS;
    
    std::vector<std::vector<double> > weights; // The weight in each interval, used to improve the grid map, size = dimensions x N_INTERVALS;
    std::vector<std::vector<double> > counts; // Count the numbers of random numbers in specific interval, size = dimensions x N_INTERVALS;
    
    std::vector<std::vector<double> > smoothed_weights; // Smoothed weights, also renormalized, size = dimensions x  N_INTERVALS
...
public:
    ...
    int Get_N_Interval() const {return N_INTERVALS;}
    ...
    double Get_Jac(const std::vector<double> &y);
